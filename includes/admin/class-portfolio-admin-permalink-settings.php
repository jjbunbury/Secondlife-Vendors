...
if ( ! class_exists( 'Vendor_Admin_Permalink_Settings' ) ) :

/**
 * Vendor_Admin_Permalink_Settings Class
 */
class Vendor_Admin_Permalink_Settings {
	...
	public function settings_init() {
		...
		add_settings_section( 'woocommerce-permalink', __( 'Product permalink base', VENDOR_UNIQUE_IDENTIFIER ), array( $this, 'settings' ), 'permalink' );

		// Add our settings
		add_settings_field(
			'woocommerce_product_category_slug',      	// id
			__( 'Product category base', VENDOR_UNIQUE_IDENTIFIER ), 	// setting title
		...
		add_settings_field(
			...
			__( 'Product tag base', VENDOR_UNIQUE_IDENTIFIER ), 	// setting title
		...
		add_settings_field(
			...
			__( 'Product attribute base', VENDOR_UNIQUE_IDENTIFIER ), 	// setting title
	...
	public function product_category_slug_input() {
		...
		?>
		<input name="woocommerce_product_category_slug" type="text" class="regular-text code" value="<?php if ( isset( $permalinks['category_base'] ) ) echo esc_attr( $permalinks['category_base'] ); ?>" placeholder="<?php echo _x('product-category', 'slug', VENDOR_UNIQUE_IDENTIFIER) ?>" />
		<?php
	}
	...
	public function product_tag_slug_input() {
		$permalinks = get_option( 'woocommerce_permalinks' );
		?>
		<input name="woocommerce_product_tag_slug" type="text" class="regular-text code" value="<?php if ( isset( $permalinks['tag_base'] ) ) echo esc_attr( $permalinks['tag_base'] ); ?>" placeholder="<?php echo _x('product-tag', 'slug', VENDOR_UNIQUE_IDENTIFIER) ?>" />
		<?php
	}
	...
	public function settings() {
		echo wpautop( __( 'These settings control the permalinks used for products. These settings only apply when <strong>not using "default" permalinks above</strong>.', VENDOR_UNIQUE_IDENTIFIER ) );
		...
		$base_slug 		= ( $shop_page_id > 0 && get_page( $shop_page_id ) ) ? get_page_uri( $shop_page_id ) : _x( 'shop', 'default-slug', VENDOR_UNIQUE_IDENTIFIER );
		$product_base 	= _x( 'product', 'default-slug', VENDOR_UNIQUE_IDENTIFIER );
		...
		<table class="form-table">
			<tbody>
				<tr>
					<th><label><input name="product_permalink" type="radio" value="<?php echo $structures[0]; ?>" class="wctog" <?php checked( $structures[0], $product_permalink ); ?> /> <?php _e( 'Default', VENDOR_UNIQUE_IDENTIFIER ); ?></label></th>
					<td><code><?php echo home_url(); ?>/?product=sample-product</code></td>
				</tr>
				<tr>
					<th><label><input name="product_permalink" type="radio" value="<?php echo $structures[1]; ?>" class="wctog" <?php checked( $structures[1], $product_permalink ); ?> /> <?php _e( 'Product', VENDOR_UNIQUE_IDENTIFIER ); ?></label></th>
					<td><code><?php echo home_url(); ?>/<?php echo $product_base; ?>/sample-product/</code></td>
				</tr>
				<?php if ( $shop_page_id ) : ?>
					<tr>
						<th><label><input name="product_permalink" type="radio" value="<?php echo $structures[2]; ?>" class="wctog" <?php checked( $structures[2], $product_permalink ); ?> /> <?php _e( 'Shop base', VENDOR_UNIQUE_IDENTIFIER ); ?></label></th>
						<td><code><?php echo home_url(); ?>/<?php echo $base_slug; ?>/sample-product/</code></td>
					</tr>
					<tr>
						<th><label><input name="product_permalink" type="radio" value="<?php echo $structures[3]; ?>" class="wctog" <?php checked( $structures[3], $product_permalink ); ?> /> <?php _e( 'Shop base with category', VENDOR_UNIQUE_IDENTIFIER ); ?></label></th>
				...
				<tr>
					<th><label><input name="product_permalink" id="woocommerce_custom_selection" type="radio" value="custom" class="tog" <?php checked( in_array( $product_permalink, $structures ), false ); ?> />
						<?php _e( 'Custom Base', VENDOR_UNIQUE_IDENTIFIER ); ?></label></th>
					<td>
						<input name="product_permalink_structure" id="woocommerce_permalink_structure" type="text" value="<?php echo esc_attr( $product_permalink ); ?>" class="regular-text code"> <span class="description"><?php _e( 'Enter a custom base to use. A base <strong>must</strong> be set or WordPress will use default instead.', VENDOR_UNIQUE_IDENTIFIER ); ?></span>
					</td>
				</tr>
			</tbody>
		</table>
...
return new Vendor_Admin_Permalink_Settings();