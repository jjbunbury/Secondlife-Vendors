...
function wc_product_post_type_link( $permalink, $post ) {
    ...
    $terms = get_the_terms( $post->ID, VENDOR_TAXONOMIES_CATEGORY );

    if ( empty( $terms ) ) {
    	// If no terms are assigned to this post, use a string instead (can't leave the placeholder there)
        $product_cat = _x( 'uncategorized', 'slug', VENDOR_UNIQUE_IDENTIFIER );
    } else {
 ...
 * @return string
 */
function wc_placeholder_img( $size = VENDOR_IMAGE_SIZE_THUMBNAIL ) {
	$dimensions = vendor_get_image_size( $size );
